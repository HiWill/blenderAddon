...
#
class denoise_baked_operator(bpy.types.Operator):
    bl_idname = 'optix_denoising.denoise_baked'
    bl_label = 'Denoise Baked'
    ...
    def execute(self, context):
        ...
        try:
            if bpy.data.images["Baked"] == None:
                show_message_box("There is no Baked to denoise. Either render manually and then denoise or use the \"Render Baked\" button.", "OptiX Denoising", 'ERROR')
                return {"FINISHED"}
        except:
            show_message_box("There is no Baked to denoise. Either render manually and then denoise or use the \"Render Baked\" button.", "OptiX Denoising", 'ERROR')
            return {"FINISHED"}

        bpy.data.images["Baked"].save_render(denoising_temp_image_path)
...
class optix_denoising_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        layout = self.layout
        row0 = layout.row()
        row0.operator("optix_denoising.denoise_baked", text="Denoise Baked")
        row1 = layout.row()
...
def register() :
    ...
    try:
        bpy.utils.register_class(denoise_baked_operator)
    except:
        print("Unable to register the class \"denoise_baked_operator\"")
    try:
...
def unregister() :
    ...
    try:
        bpy.utils.unregister_class(denoise_baked_operator)
    except:
        print("Unable to unregister the class \"denoise_baked_operator\"")
    try:
